class GenerateDailyTasksForPlantJob < ApplicationJob
  ...
  def generate_plant_watering_task
    ...
    if past_rain_array.last(days).all?(0) && today_rain.zero? && garden_plant.tasks.where(activity: "Arrose-moi !").where("done_date > :limit_date", limit_date: Date.today - days.days).count.zero?
      Task.create(activity: "Arrose-moi !", criticity: "low", due_date: DateTime.now + 1.day, start_time: DateTime.now, garden_plant: @garden_plant)
    end
  end
  ...
  def generate_plant_displacement_task
    ...
    elsif @forecast_min_temp_array.select { |temperature| temperature < -5 } && garden_plant.tasks.where(activity: "Rentre-moi à l'intérieur !").count.zero?
      Task.create(activity: "Rentre-moi à l'intérieur !", criticity: "medium", due_date: DateTime.parse(@forecast_dates_array[@forecast_min_temp_array.index { |temperature| temperature < -5 }]), start_time: DateTime.now, garden_plant: @garden_plant)
    end
  end
  ...
  def generate_plant_cover_task
    ...
    elsif @forecast_min_temp_array.select { |temperature| (-5..0).cover?(temperature) } && garden_plant.tasks.where(activity: "Couvre-moi !").count.zero?
      Task.create(activity: "Couvre-moi !", criticity: "medium", due_date: DateTime.parse(@forecast_dates_array[@forecast_min_temp_array.index { |temperature| (-5..0).cover?(temperature) }]), start_time: DateTime.now, garden_plant: @garden_plant)
    end
  end
  ...
  def generate_mulching_task
    ...
    elsif @forecast_min_temp_array.select { |temperature| (0..5).cover?(temperature) } && garden_plant.tasks.where(activity: "Paille-moi !").count.zero?
      Task.create(activity: "Paille-moi !", criticity: "medium", due_date: DateTime.parse(@forecast_dates_array[@forecast_min_temp_array.index { |temperature| (-5..0).cover?(temperature) }]), start_time: DateTime.now, garden_plant: @garden_plant)
    end
  end
end
